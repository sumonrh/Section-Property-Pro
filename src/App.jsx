...
const ResultRow = ({ label, value, unit, highlight, warning }) => (
  <div className={`flex items-center justify-between p-2.5 rounded-lg border transition-all ${highlight ? 'bg-blue-50 border-blue-100 text-blue-900' : 'bg-slate-50 border-slate-100 hover:bg-white'}`}>
    <span className={`text-xs flex items-center gap-1 ${highlight ? 'font-bold' : 'font-medium text-slate-600'}`}>
      {label}
      {warning && <AlertTriangle size={12} className="text-amber-500" title={warning} />}
    </span>
    <div className="text-right">
      <span className={`text-sm font-mono font-bold ${highlight ? 'text-blue-700' : 'text-slate-900'}`}>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-8 font-sans text-slate-900">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200/60">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium text-sm">Interactive Section Geometry Evaluator</p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={handlePrintReport}
              disabled={!results || isSolvingTorsion}
              className="flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-8 py-3 rounded-2xl font-bold transition-all shadow-lg shadow-slate-200 active:scale-95"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <aside className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-slate-200 p-1.5 rounded-2xl flex font-bold text-sm shadow-inner">
              <button
                onClick={() => setActiveMode('parametric')}
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl transition-all ${activeMode === 'parametric' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Layers size={16} /> Library
              </button>
              <button
                onClick={() => setActiveMode('freedraw')}
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl transition-all ${activeMode === 'freedraw' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
            ...
            {activeMode === 'parametric' ? (
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/60 space-y-6">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 text-slate-800 font-bold p-3 rounded-xl appearance-none cursor-pointer outline-blue-500"
                  >
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-1 bg-slate-50 rounded p-1">
                            <input
                              ...
                              className="w-16 bg-transparent border-none text-right font-mono text-xs font-bold text-blue-600 focus:outline-none"
                            />
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/60 space-y-6">
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl">
                  <h3 className="text-sm font-bold text-blue-900 flex items-center gap-2 mb-2"><MousePointerSquareDashed size={16} /> Drawing Guide</h3>
                ...
                <div className="space-y-3 pt-2">
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-4 rounded-xl border-2 transition-all font-bold ${fdTarget === 'outer' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 hover:border-slate-200 text-slate-600'}`}
                  >
                    <div className={`w-4 h-4 rounded-full border-2 ${fdTarget === 'outer' ? 'border-blue-600 bg-blue-400' : 'border-slate-400'}`}></div>
                    Draw Boundary
                  </button>
                  <button
                    onClick={() => setFdTarget('hole')}
                    className={`w-full flex items-center gap-3 p-4 rounded-xl border-2 transition-all font-bold ${fdTarget === 'hole' ? 'border-red-600 bg-red-50 text-red-700' : 'border-slate-100 hover:border-slate-200 text-slate-600'}`}
                  >
                    <div className={`w-4 h-4 rounded-full border-2 ${fdTarget === 'hole' ? 'border-red-600 bg-red-400' : 'border-slate-400'}`}></div>
                    Draw Hole / Cutout
                  </button>
                </div>
          ...
          <main className="lg:col-span-8 space-y-6">
            ...
            {geometry.error && (
              <div className="bg-red-50 border border-red-200 text-red-800 p-6 rounded-3xl flex items-center gap-4 animate-in fade-in">
                <AlertCircle size={24} />
                <div>
            ...
            {results && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
                ...
                <section className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200/60 space-y-2">
                ...
                <section className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200/60 space-y-2 lg:col-span-1 md:col-span-2">
                  <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3 flex items-center gap-2">
                  ...
                  {activeMode === 'freedraw' && torsionData && (
                    <div className="mt-4 p-3 bg-slate-50 border border-slate-100 rounded-xl">
                      <p className="text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wide">FEM Diagnostics</p>
